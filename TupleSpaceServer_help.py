...
def handle_client(client_socket):
    ...
    try:
        while True:
            ...
            message_buffer = receive_n(client_socket, msg_len - 3)
            ...
            total_response_len = len(response_body) + 3     
            response_message = f"{total_response_len:03d}{response_body}"
    ...
    with lock:
        if op == "R":
            ...
            if len(value) > 999 or len(f"{key} {value}") > 970:
                increment_stat("error_count")
                return "ERR Value too long or total tuple size exceeds limit"
            
            # Check if the key already exists.
            if key in tuple_space:
                return f"ERR {key} already exists"
            else:
                # Add key-value to tuple_space
